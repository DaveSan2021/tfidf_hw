...
class ToyTfIdfGuesser(Guesser):
    ...
    def __init__(self, filename, max_vocab_size=10000,
        ...
        # Add your code here! 
        self.seen_words = defaultdict(int) #for reading vocabulary into model
        self.document = {} #holding words in each document, holds dictionary pairing of {doc number -> {word:freq}}
        self.terms_all_documents = defaultdict(int) #gloabl frequency table of words
        self.total_words = 0 #total words in all the docs
    ...
    def __call__(self, question, max_n_guesses=1):
        ...
        for prompt in self.questions:
            prompt_tfidf = self.embed(prompt).reshape(1, -1)
            # dot_product = np.dot(prompt_tfidf, np.reshape(question_tfidf, (42, 1)))[0][0]
            # norm_product = np.linalg.norm(prompt_tfidf)*np.linalg.norm(question_tfidf)
            # similarity= dot_product/norm_product
            cosine.append(cosine_similarity(prompt_tfidf,question_tfidf)[0][0])
    ...
    def scan_document(self, text: str):
        ...
        document_terms = defaultdict(lambda:0)

        for word in tokenized:

            #kunk tokens all have frequency of 1
            if word==0:
                document_terms[word] = 1
            else:
                document_terms[word] += 1

            self.terms_all_documents[word] += 1
            self.total_words += 1
        
        self.document[self._total_docs] = document_terms
        self._total_docs += 1
    ...
    def global_freq(self, word: int) -> float:
        ...
        """
        if word not in self._vocab.values():
            return 0.0
        
        return self.terms_all_documents[word]/self.total_words 
    ...
    def inv_docfreq(self, word: int) -> float:
        ...
        if word not in self._vocab.values():
            ...
            dn = 0
            for word_map in self.document.values():
                if word in word_map:
                    dn += 1
        if dn==0:
            return 0.0
        return np.log10((float(self._total_docs)/float(dn)))
    ...
    def finalize_vocab(self):
        ...
        for word, freq in self.seen_words.items():
            if freq >= self._unk_cutoff:
                self._vocab[word] = id
                id+=1
